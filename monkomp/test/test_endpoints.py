...
class APITest(unittest.TestCase):
	...
	def test_customer(self):
		...
		# Create new customer
		response = self.client.post('/newcustomer', json=customer)
		...
		self.assertEqual(response.get_json(), {"error":"Bad Request","message":"Customer with this email already exists."})

		# Try to create customer with invalid data
		invalid_data = 12
		response = self.client.post('/newcustomer', json=invalid_data)
		self.assertEqual(response.status_code, 400)
		...
		empty_customer = {}
		response = self.client.post('/newcustomer', json=empty_customer)
		self.assertEqual(response.status_code, 400)
